...
discount — the fang anchor), or `▽ caution — dip into an UNPROVEN base, not near durable support` (2026-08-06 — the SHAPE read says rising while floorValidator's 28d LEVEL read still cautions; a COMPOSITION of the two existing checks, not a new one). Inform-only PLACEHOLDER (n≈0), stdout-only — never gates/drops/regrades and never enters
...
| `floorValidator` | the durable multi-week floor (`termStructure`) + (R3) the daily-mid `recentTrend` | how far above the 28d floor the buy sits, in the item's own typical-swing (IQR) units: **> 1.5 swings → caution** (`FLOOR_CAUTION_RANGES`, **MEASURED 2026-08-08** — was 1.0; the 1.0–1.5 band carried P(drawdown ≥ 1 swing) of 12.0% vs 29.8% above it, so moving the line silenced 69.6% of firings and kept the signal). The reject tier (2.0) is **inert** — nothing exceeded it in 35 days. It predicts **drawdown, not loss**: 7-day returns are flat across every elevation bucket, and the floor it names prints only 6–8.5% of the time in 48h, so the reason states the expected dip, not "durable support". (R3, additive-only) a falling `recentTrend` tightens an already-elevated buy (caution→reject, borderline-pass→caution) — **unsupported at n=60 and the only way floor drops rows**; never relaxes a clean low pass |
