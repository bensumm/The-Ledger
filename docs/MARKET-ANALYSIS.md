...
discount — the fang anchor; EC2 2026-09-08: a forming day already under the prior trough joins this branch —
...
which beats the shipped 2h breakdown only at miss-cost ratio ≈3; the full-line render appends the FIRING half —
`(cushion + pace)` and/or the `highs under 7d profile Nh (−X gp)` magnitude — never the quiet one). Inform-only (the fade caution carries HF1's measurement; the rest n≈0), stdout-only — never gates/drops/regrades and never enters
