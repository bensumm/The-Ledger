...
| E1b | Every SCREAMING_SNAKE constant a pipeline entrypoint USES is imported or declared | `pipeline/ci/check-imports.mjs` (Part 2) | a `ReferenceError` at runtime — the reverse of E1, and invisible to it, to `node --check`, and to the suite when no test executes the function. |
...
| E3 | Docs carry no superseded terms; no single-source phrase is duplicated across the CLAUDE.md⇆README axis; no governed doc quotes a stale literal beside a live constant's name | `pipeline/ci/lint-docs.mjs` (+ `.test.mjs`) | a denylisted term (e.g. a deleted flip-niche as "live"), a duplicated invariant, or a number that drifted when its constant moved (the dated record — CHANGELOG/LORE/plans — is deliberately ungoverned) |
...
| Watchlist membership (the permission/priority set) | `pipeline/lib/config/watchlist.mjs` (`loadWatchlistIds`, `loadWatchlistEntries`, `loadWatchlistNames`) | SEP16a — five node loaders parsed `watchlist.json` separately. Membership is not decoration: it grants gate exemption, the bounded band-stack fetch reserve, the unbounded amplitude reserve, the amplitude proxy-floor bypass, `subFloorFallback`, the sub-break-even render exemption, the `NOISE_OFFER_GP` incidental-lot exemption in `quote-items.mjs` + `watch-positions.mjs`, and two big-ticket force-includes — all of which turn off together, silently, if the set empties. Role metadata lives in the `watchlist-meta.json` sidecar and NEVER feeds membership (the app rewrites `watchlist.json` wholesale on every star-click, so nothing durable can live in it). `js/ui.js`'s `loadRepoWatchlist` is the app's own `fetch` of the same file and cannot import this node module — the one reader covers the pipeline, not the browser. |
| Sync-before-read invocation (SY1) | `pipeline/lib/reconstruct/sync-invoke.mjs` (`runLocalSync`) | `screen-flip-niches.mjs`, `quote-items.mjs`, `watch-positions.mjs`, `read-book.mjs` each call it once (AR1 — was copy-pasted byte-for-byte, with a hairline regex divergence, across the first three) |
| Fetch-pool admission (screen) | `pipeline/lib/signal/admission.mjs` (`pickFetchPool`, default) / `pipeline/lib/signal/gatecandidates.mjs` (`rankAndSlice`, `--admission legacy` rollback) | `screen-flip-niches.mjs`. Exploration-reserve honesty note: `pickFetchPool`'s exploration reserve is `Date.now()`-bucketed (deliberately left non-deterministic), so a survivor admitted purely on this pass's rotation carries `via:'explore'` and the screen table marks it 🎲 — a lottery slot is never rendered as a ranked-in pick. Inform-only; never gates/ranks/grades. **SUNSET CONDITION for the `legacy` path — EVIDENCE-based, not time-based:** `rankAndSlice` retires once a `join-outcomes` read shows `unified` non-regressive against it; until then any gate-stack change (a new reserve category, a new filter) MUST be applied to BOTH functions or `legacy` silently diverges further on every one-sided edit. Keeping it costs nothing to run and everything to maintain, so the trigger is the proof, not the calendar — the same shape as the reserve EXIT CONDITION in `admission.mjs`. |
...
Full audit + findings: `PLAN-ARCH-DOCS-AUDIT.md` (Parts 1–5); the disposition validation: PLAN.md's
`CV1–CV14` Status row.
