...
| `windowReliability` | does this item's intraday SHAPE reproduce? parity split-half Pearson r on `hourProfile`'s de-trended devLow/devHi; gate = `min(rLow,rHi) ≥ 0.6` | 1h, own PINNED 14d window (never the caller's `nights` — at 7d it is uncomputable for ~100% of items) | GATES DISPLAY of the diurnal hours on every surface; gates nothing else | ⚠️ PLACEHOLDER threshold, n≈0 for gp; measures within-day HOUR RANKING, not fill probability; the render fit is 7d while the gate judges 14d (agree on the dip hour ~34%) | `diurnalTimedLap`, `formatTimedLap`, `softBuyRead`, digest, `read-schedule`, `read-window-range --profile` |
...
| **Decision digest** `digestVerdict` | one-word triage (`sell unreliable` / `mirage top` / `weak deploy` / `starter…` / `fill-now` / `low-conviction`) | `reachFrac` (RC1 recent-3 — since RB-5 the shared `reachFraction(_, {prefer:'recent'})`, not a local copy), `askPlacement` (whole-window CDF), `phase` (diurnal in/pre/post-peak), `weakDeploy` (ROI%) | every non-held, non-sub-floor candidate, `--digest` opt-in | ✅ for reach, ❌ for placement, n/a for phase (clock, not trend). **Expect the digest's reach column to disagree with the published `screen.json` rank/grade on a recency-divergent row** — the digest is a DISPLAY surface (recent-3) and the rank is still full-window; that is decided, not drift | `screen-flip-niches.mjs --digest` |
