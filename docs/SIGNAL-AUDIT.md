...
| `estimatePair`/`reachFoldModel` (R5 trend-aware) | reconciliation Est.buy/Est.sell: buy = band-low/live/trough per doctrine; **sell = live + (bandTop−live)×reach-FOLD FACTOR × R5 fade**; plus the `pFill` printed beside the net | reach-fold factor = `min(1, askFrac/0.75)` (askFrac = the shared `reachFraction`, **recent-3** preferred), **× `EST_FADE_DISCOUNT` when the ask-side `reachMargin.trend` is `fading`** (R5 — via `extra.askMargin`, screen-wired); **`pFill` = `askReachFactor` on the SAME recent-3 basis since RB-3** (it was full-window before, contradicting the price beside it) | default table column on quote + screen | ✅ recent-3 fold AND its P now share one basis (RB-3); (R5) a slope-based cushion-fade tightens a clean-reach mirage. NOT shared with the RANK's P — see the `askReachFactor` row | quote-items (no fade yet), screen niche tables + digest |
...
| `askReachFactor` | two-leg exit-fill discount off ask-reach fraction, floored at 0.25 | **caller-declared basis** via `reachFraction(askReach, { prefer })` — `prefer:'full'` (the DEFAULT) = `reachedDays / nDays`; `prefer:'recent'` = `recentHit / recentDays`, degrading to full when no recent counts exist | every rank except `symmetric`-fillShape (churn/amplitude); the `estimatePair` display P; watch-positions' size-relief note | ⚠️ **SPLIT BY CALL SITE since RB-3 (2026-08-04) — and the split is deliberate.** DISPLAY sites (`estimatePair`'s `pFill`, `read-window-range`'s fold line, watch-positions' relief note) opt into **recent-3 ✅**, so the probability matches the recent-3 fold PRICE printed beside it — before RB-3 the price was recent-3 and the probability full-window and the two contradicted each other on any regime-changed item. The **RANK** (`js/estimators/families.mjs`) is **still full-window ❌**: a measurement pass found the rank-basis swap moves the composite rank >33% on ~23% of item-days, and a four-valued n=3 probability *multiplier* is not the same risk as a continuous band-bounded price, so it is gated on a realized-fill study (PLAN-RECENCY-BASIS RB-4, deferred). Consequence to read correctly: on a recency-divergent row the Est-cell `P(ask)~` and the Rank-cell `P~` legitimately differ. Also SOFT-FLOORED — a stale mirage keeps ≥25% weight, and on the recent basis a 0/3 read hits that floor more often | `estimateRank` (full) · `estimatePair` (recent) |
| `reachFraction` (RB-3) | **the ONE recency-basis rule** — one home for "recent-3 preferred, full-window backstop", after it existed as three independent copies (the fold price in `estimatePair`'s `reachRead`, `digestReachFrac` in the screen, and an inline `reachedDays/nDays` in `askReachFactor`) | the `askReach`/`bidReach` counts already in the caller's hand (zero new fetch) | `askReachFactor`, `reachRead`, `digestReachFrac` | n/a — it *is* the recency switch; it never picks a side, the caller declares one | `js/estimators/reach.mjs` |
