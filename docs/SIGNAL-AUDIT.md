...
| `softBuyRead`/`digestSoftBuy` | is live at/near the diurnal dip floor right now — a LEVEL cue (DT2 2026-08-09: the dip window is shown for ATTENDED taking only; it does NOT time a resting offer — 71.2% in-window touch vs 70.5% random, so waiting forfeits ~29% of fill-days at the same price) | derived from `hourProfile`; hours clause gated by `windowReliability` (DT4) | held-lot ADD timing (quote), digest buy column | inherits `hourProfile` | quote notes, digest |
...
| `floorValidator` | buy-level distance above the durable floor, in typical-swing units | `termStructure` | buy-side gate | ⚠️ inherits termStructure's staleness, BUT **forward-scored 2026-08-08 (n=4,121)**: the distance predicts DRAWDOWN monotonically (Spearman ρ 0.151, within-item p=0.066) though NOT loss (7d return flat). Honest effect size **~8–11pp** on a fixed-percent threshold (P(depth ≥ 8%) 12.9%→15.3%→23.9% raw min, 10.2%→11.7%→18.6% robust — the swing-unit numbers overstate ~2× because the outcome shares terms with the bucketing). Caution line retuned 1.0 → **1.5** on that study (a precision/recall trade: 69.6% of firings silenced, recall kept 52%); reject tier **unmeasured in band/churn** (censored by its own gate — a reject row never reaches the ledger); the named floor prints only 6–8.5% of the time in 48h | validator registry |
...
| `dipPostureValidator` | wraps `recentDirection` into pass/caution — an ENTRY-QUALITY note since 2026-08-08 | live + 5m/3h | dip buy candidates | ✅ **forward-scored 2026-08-08 (n=5,535) — the original "reverting ⇒ the bid misses" claim was FALSIFIED** (reverting reached 85.7% @8h vs falling 82.6%); the cross-or-pass advice was removed. The quoted level is `quickBuy`, which rises with the bounce | validator registry |
...
| `askReachFactor` | two-leg exit-fill discount off ask-reach fraction, floored at 0.25 | **caller-declared basis** via `reachFraction(askReach, { prefer })` — `prefer:'full'` (the DEFAULT) = `reachedDays / nDays`; `prefer:'recent'` = `recentHit / recentDays`, degrading to full when no recent counts exist | every rank except `symmetric`-fillShape (churn/amplitude); the `estimatePair` display P; watch-positions' size-relief note | ⚠️ **SPLIT BY CALL SITE — REDUCED 2026-08-09, not eliminated.** `estimatePair`'s `pFill` and the `read-window-range` fold line flipped **back to full-window ✅**, joining the RANK — so the main price/probability/rank triple is now ONE basis and RB-3's display-vs-rank caveat is retired. Still recent-preferring, and now the ONLY sites that are: the `--digest` reach column (`digestReachFrac`) and watch-positions' size-relief note — ✅ **MEASURED 2026-08-13** (`join-reach-basis.mjs`): recent-3 is the cheaper basis for the digest column (M(1)=+2.3pp, item-clustered CI [0.8,3.8], stable across four horizons and both eras), so it STAYS recent and the split is now deliberate rather than unexamined. ⚠ The same run's headline is about the TAG, not the basis: below r≈1.29 both bases lose to not gating at all, so `sell unreliable` is a prompt, never a filter. The **RANK** (`js/estimators/families.mjs`) is **full-window ❌**: a measurement pass found the rank-basis swap moves the composite rank >33% on ~23% of item-days, and a four-valued n=3 probability *multiplier* is not the same risk as a continuous band-bounded price, so it is gated on a realized-fill study (PLAN-RECENCY-BASIS RB-4, deferred). ~~Consequence to read correctly: on a recency-divergent row the Est-cell `P(ask)~` and the Rank-cell `P~` legitimately differ.~~ **RETIRED — this sentence contradicted its own cell and survived the 2026-08-09 GLOSSARY fix one file over. Display and rank are BOTH full-window (`js/estimators/pair.mjs:128,:266` use `{prefer:'full'}`), so they cannot differ.** Also SOFT-FLOORED — a stale mirage keeps ≥25% weight, and on the recent basis a 0/3 read hits that floor more often | `estimateRank` (full) · `estimatePair` (**full** since 2026-08-09; this column said `recent`) |
| `reachFraction` (RB-3) | **the ONE recency-basis rule** — one home for the recent-vs-full choice (the CALLER declares; the function never picks), after it existed as three independent copies (the fold price in `estimatePair`'s `reachRead`, `digestReachFrac` in the screen, and an inline `reachedDays/nDays` in `askReachFactor`) | the `askReach`/`bidReach` counts already in the caller's hand (zero new fetch) | `askReachFactor`, `reachRead`, `digestReachFrac` | n/a — it *is* the recency switch; it never picks a side, the caller declares one | `js/estimators/reach.mjs` |
...
| **Decision digest** `digestVerdict` | one-word triage, evaluated top-to-bottom, first match wins: `spread closed now` / `mirage top` / `mirage top!` / `weak deploy` / `starter / hold-to-next-peak` / `fill-now` / `low-conviction`. **`sell unreliable` was DELETED (SEP12)** — it fired at priority 1 while measured to lose to not gating at all below cost ratio ~1.29, and rows were dismissed on it unread; the reach FRACTION and its basis are displayed instead. Do not re-add it | `crossable` (live spread, `false` only — `null` is unknown-neutral), `reachFrac` (RC1 recent-3 — since RB-5 the shared `reachFraction(_, {prefer:'recent'})`, not a local copy), `askPlacement` (whole-window CDF), `placementDiverges` + `marginTrend` (the `!` escalation, needs BOTH), `phase` (diurnal in/pre/post-peak), `weakDeploy` (ROI%) | every non-held, non-sub-floor candidate, `--digest` opt-in | ✅ for reach, ❌ for placement, n/a for phase (clock, not trend). **Expect the digest's reach column to disagree with the published `screen.json` rank/grade on a recency-divergent row** — the digest is a DISPLAY surface (recent-3) and the rank is still full-window; that is decided, not drift | `screen-flip-niches.mjs --digest` |
