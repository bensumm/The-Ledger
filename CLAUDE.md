...
| "did the **reach surface** actually work?", "does **askStar** beat what we have?", "which **exit ask** makes the most gp?", "**backtest** the exit price" | `node pipeline/commands/join-exit-ev.mjs [--items N] [--stride H] [--horizon H] [--bail low\|high] [--json]` (2026-08-29, PLAN-REACH-SURFACE chunk 4 — the decisive backtest: at each archive origin, list at each contender's ask for H hours, credit `net(ask)` if the market reaches it, else the bail. **THE PRE-REGISTERED NULL BRANCH FIRED — read it before quoting anything off the reach surface: `askStar` LOSES to an incumbent, so the surface is a DESCRIPTION layer, the chunk-5 default swap is CANCELLED, and `read-exit-surface.mjs` prices nothing you should trade off.** The two incumbents at the top (`asym`, `reachFold`) are TIED, and the surface lands below both AND below the live instabuy. It also NOMINATES estimators for exit-pricing retirement under a criterion pre-registered in the file header — chunk 8 executes, a nomination is not a retirement, and the BID side survives. Inform-only, gates nothing; the ASK leg only. Full result, the acceptance check that once inverted it, and the three sensitivities: README's `join-exit-ev.mjs` entry — don't restate them here) |
