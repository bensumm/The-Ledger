...
| "is **X** a **falling knife**?", "**stepping down** by the hour?", "does the ask still **clear midday**?" | `node pipeline/commands/read-window-range.mjs "<item or id>" --hourly --days 3` (PLAN-HOURLY-3DAY-TREND HT0/HT1 — the per-hour day-over-day drift + Δ/d column + ask-reachability-decay; DONE — HT0–HT3+HT5 shipped, HT4's reverse-flip fold DEFERRED pending PLAN-REVERSE-FLIP). Auto-surfaced without asking on every quote/positions/scan-digest read too — see `docs/MARKET-ANALYSIS.md` §5) |
