...
| "does the **fade alert** measure anything?", "score **hours-under-profile**", "is the **FADE** trigger real?", "re-run the **fade** measurement" | `node pipeline/commands/join-fade-outcomes.mjs [--items N] [--cost-ratio r] [--item "<item>"] [--json]` (2026-09-22, PLAN-HOLD-FADE-ALERT HF1 — the pre-registered scorer behind the watch `FADE` alert's hours half. **Read the result before quoting it: the hours trigger beats the shipped 2h-breakdown replay ONLY at miss-cost ratio r=3 (k ∈ {2,3,4}, CI-supported) and loses decisively at r ≤ 2; both signals lose to never-alert below r ≈ 2.5; the big-ticket stratum is where it is weakest (r\* ≈ 7–8).** `FADE_MIN_HOURS` = the largest winning k. Inform-only, gates nothing. Full result + the replay-approximation caveats: README's `join-fade-outcomes.mjs` entry — don't restate them here) |
