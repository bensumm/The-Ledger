...
| R6 | Retire `trajectoryRead`'s printed shape label; fold its unique fields (floor/ceiling/livePos) into the `floorCeilingTrack`/`projectTrajectory` note | `js/windowread.mjs`, `pipeline/commands/quote-items.mjs`, `pipeline/commands/read-window-range.mjs`, `pipeline/test/windowread.test.mjs`, skills (`positions` 1.48, `scan` 1.83) | R1, R2 | inform (display-only) | **LANDED** — Fable-reviewed TWICE (usefulness/design review → Ben chose "fold + keep oscillation"; then impl review → no issues). Shape line retired on all 3 print sites (quote-items pushTrajectory, read-window-range --trajectory + --profile); floor/ceiling band + livePos fold into the fcTrack note (`formatFloorCeiling`'s `live` opt); the one unique signal — oscillation DENSITY — migrated to `fc.oscillating` (rendered `ranging (oscillating floor↔ceiling)`). `trajectoryRead` code kept (no consumer read `shape`). No APP_VERSION bump (the app's floorCeilingTrack consumer reads only `.classification`, unchanged; formatFloorCeiling is pipeline-only). 73 windowread checks (4 new R6) + full suite green. Live render verified. |
| R7 | Add a single `cappedBy` field to the screen row naming which of the grade ceilings (if any) bound the printed letter | `js/rating.mjs`, `pipeline/commands/screen-flip-niches.mjs`, `pipeline/lib/suggestlog.mjs`, tests (`rating`, `suggestlog`) | — (independent) | n/a (legibility only) | **LANDED** — Fable-reviewed (2 gaps fixed: amplitude + watchlist `suggestionEntry` calls now thread `cappedBy` too, so ledger coverage is consistent across all 3 `rateItem` sites). `rateItem` names the THIN cap; the screen's `cap(g,ceiling,name)` helper names PHASE_BASING/SUBFLOOR/REACH last-binder-wins; logged to suggestions.jsonl (YS2 lean) + grade-cell tooltip. `rateItem` grade output byte-identical (pure refactor) so NO APP_VERSION bump (js/market.js reads only .grade/.score; the new field + tooltip are pipeline-generated data through unchanged app code — Fable-confirmed). Live: cappedBy logs across all paths. Full suite + 2 new test cases green. |
| R8 | Decide + document the fate of DE1/DE6 depth pricing and the Extension-B pressure/demand read | `js/windowread.mjs`, `pipeline/commands/{read-window-range,screen-flip-niches}.mjs`, `pipeline/lib/suggestlog.mjs`, tests, docs | — | removal | **RESOLVED — Ben chose REMOVE (narrow).** Fable mapped the consumer graph (`PLAN-REMOVE-DEPTH-PRESSURE-READS.md`) and found R8's "inspector-only" premise STALE — `clearableAsk`/`reachableBand` have had live shadow consumers since 2026-07-15. Narrow removal EXECUTED: deleted `depthDays`+`clearableBid` (DE1/DE6 depth) and `hourlyPressure`+`demandRegime` (Extension-B demand-cycle) + their inspector blocks/shadow field/tests; KEPT `clearableAsk`/`demandPressure`/`reachableBand` (they power the `--est-sell pressure` sell-model + the DE3 depthExit shadow + Gate B). Chunk 4 (broad — kill the pressure model too) declined. git-revivable. |
