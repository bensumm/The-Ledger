...
Status: **Chunks 1–6 + F-A..F-H + Wave-3 Phase 1 SHIPPED; W3-3 and Wave-3 Phase 2 remain OPEN (DEFERRED).**
Every shipped chunk's sha is in the Status table below; #4, #6 and F-I are SHELVED.
⚠ Before widening the detector
window, read F-H's length-trap note — a deeper series takes `oscillationVsKnife` to ~100% OSC and
silently deletes the Chunk-3B knife temper.
...
**F-B — the crowded-out-fetch fix, SINCE LANDED.** The audit found fang/dragon boots/blowpipe ranked
below the then-25 amplitude-proxy fetch cut, so they never reached the (now working) margin gate on a
normal scan. Both halves of the fix shipped: F-B's watchlist RESERVE (`62da022`) guarantees a fetch
slot below the top-N cut for a `watchlist.json` id, and F-D raised `AMP_TOP_DEFAULT` to **40**
(`pipeline/lib/signal/gatecandidates.mjs:131`). The target class now reaches the gate.
...
### F-A — LANDED `62da022` (Fable, 2026-07-22): `oscillationVsKnife` redesigned
...
### F-B — LANDED `62da022` (Fable, 2026-07-22): amplitude watchlist RESERVE
...
### F-C — LANDED `481f2e9` (Fable, 2026-07-22): per-thesis HOLD HORIZONS
The main session's own audit of F-A/F-B flagged a real GAP: `driftExitFrom`'s `holdHorizonDays`
defaulted to `OSC_HOLD_HORIZON_DAYS=1.5` (the oscillation-forecast default; NOT the amplitude hold, which DT1 later re-horizoned to 4d) at EVERY call site
...
| F-C | ✅ LANDED | `481f2e9` | per-thesis HOLD HORIZONS for the drift-adjusted-exit note (the audit's own GAP finding) — `DRIFT_INTRADAY_HOLD_DAYS` (band/churn/scalp, ~2h, anchored to `BAND_HOURS`) and `DRIFT_VALUE_HOLD_DAYS` (value, 14d, anchored to `termstructure.mjs`'s `FLOOR_FALLBACK_DAYS`) replace the blanket 1.5d default at the two Chunk-6 call sites that KNOW their thesis. Generic contexts with no known thesis (quote-items/read-window-range/trends.js/watch-positions non-cycle) deliberately kept on the default — the render already shows the real horizon used, so nothing is silently mis-scaled. No APP_VERSION bump (`js/flip-niches.mjs` still not app-imported; `js/trends.js` unchanged). Pinned by 4 new checks in `oscillation-thesis.test.mjs`. |
...
- **W3-1 — crossability / ghost-spread demotion (the biggest single denoiser).** ✅ **LANDED `22f4df7`**
...
- **W3-2 — drift-margin into the digest RANK (the reshaped F-I).** ✅ **LANDED `22f4df7`** (2026-07-22).
