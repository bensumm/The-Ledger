...
|---|---|---|---|---|
| DT1 | **Amplitude: stop grading a 1-day cycle** | Print realized completion rate per row; re-horizon to 4–7d or shelve the grades; replace `pFill2leg` (independence measured false) | `js/amplitudescreen.mjs`, `screen-flip-niches.mjs --mode amplitude`, `--mode all` | Highest value, changes a live grade |
| DT2 | **Resting-offer cue: drop "wait for the window"** | `softBuy` reworded level-based; window advice scoped to attended market-taking | `softBuyRead`/`formatSoftBuy` (`js/windowread.mjs`), `/positions` + `/scan` skills | Doctrine change |
