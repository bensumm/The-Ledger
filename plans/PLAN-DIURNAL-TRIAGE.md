...
Status: DT1, DT1b, DT2, DT3 SHIPPED (2026-08-09); DT4 SHIPPED (2026-08-10, as option B); DT4b SHIPPED (2026-08-10 — closes the fit-window transfer gap DT4 flagged); DT4c SHIPPED (2026-08-10 — closes DT4b's two open review findings: the routing convention, and the app gating its ★ on a measured non-discriminator).
DT5–DT6 NOT authorized — proposed only, Ben's call. Do not delete this file until DT5–DT6 resolve.
...
for a resting offer**. The **amplitude lane's 1-day-cycle premise is refuted** — it was surfacing
S+/A- grades on rows whose cycle had essentially never completed in-horizon, which DT1's re-horizon
and DT1b's measured per-item round-trip rate together fixed (Saturated heart A- → D on 0/37). And the
...
|---|---|---|---|---|
| DT1 | ✅ **SHIPPED 2026-08-09 (0.71.5)** — **Amplitude: stop grading a 1-day cycle** | Re-horizoned `AMP_HOLD_DAYS_DEFAULT` 1 → 4; `pFill2leg` DELETED (independence measured false). Its intended replacement `cycleCompletion` was built, measured on the live board, and **REJECTED as a rank input** — saturated by construction (median-vs-median over a multi-day horizon ≈94%; board read 18/19 incl. Saturated heart 5/5 against a study measuring it 0%/96h). `pFillAmplitude` now returns the bare 0.5 prior at n=0; the figure survives DISPLAY-ONLY as the asymmetric `ask-reprints X/Y`. **Root cause CORRECTED same day: circular in-sample levels, not sub-day grain — the study re-runs and reproduces EXACTLY (Saturated heart 0.0% @96h n=41; Masori chaps 12.9% @24h n=31), so the re-horizon is well-founded.** | `js/amplitudescreen.mjs`, `js/estimators/families.mjs`, `screen-flip-niches.mjs` | Highest value. The measured P(fill) it left open was restored by **DT1b**, shipped same day |
| DT1b | ✅ **SHIPPED 2026-08-09 (0.71.6)** — **Amplitude: a MEASURED P(fill) at last** | `ampWalkForward` (`js/amplitudescreen.mjs`) — levels fitted STRICTLY PRE-ORIGIN over the preceding 20d, entry = first hour of the origin day touching the bid, completion = any later hour reaching the ask inside the horizon, end-of-series entries PENDING. Reads the local 1h archive via `archiveSeries` on a lazy READONLY handle (~20ms/item, ~0.3s/board; the live `/timeseries` ~15d is too short). `pFillAmplitude` ranks on it above `AMP_WF_MIN_JUDGED`, prior below. `ask-reprints` withdrawn from the table (shadow-log only). **Live effect: Saturated heart A- → D on a measured 0/37**; Fury 48% held A-. Feasibility pre-verified: 227/228 items ≥5m carry ≥35d history. Also closed two live inaccuracies — the `fillShape:'symmetric'` ask-reach exemption (unsound while pFill was a prior) and a self-contradicting `pFillAmplitude` header | `js/amplitudescreen.mjs`, `js/estimators/families.mjs`, `screen-flip-niches.mjs`, `suggestlog.mjs`, `/scan` 1.99 | Acceptance fixture FAILS on a regression to in-sample fitting (downtrend reads 0% pre-origin, ≫0 in-sample) |
...
| DT4 | ✅ **SHIPPED 2026-08-10** — **Per-item window gate** | `windowReliability` (js/windowread.mjs): parity split-half Pearson r on hourProfile's de-trended devLow/devHi, gate on `min(rLow,rHi) ≥ 0.6`. Shipped as Ben's **option B — the LEVELS always render, only the HOURS are gated**, with "hours repeat most days" marking the items that pass. **The spec in this row was WRONG and is corrected in the source:** "on the in-hand series" is not implementable — at the `nights=7` window these surfaces profile at, the parity halves fall under `HOURPROFILE_MIN_DAYS` and the gate is uncomputable for **4,156 of 4,156** items, so it PINS its own 14-day window off the raw series. The 14d gate is a different selection from the study's 30d one (only 25% overlap), so its lift was re-measured directly: PASS n=40 dip +17.3pp / peak +17.4pp vs FAIL +3.3 / +4.4, gap +14.0pp ± 2.9 (t≈4.9), monotone across all five buckets. It also **displaced `hourConcentration.clean`**, which was measured NOT to discriminate (clean=true +5.0pp vs clean=false +3.6pp dip) yet was withholding the BID/ASK levels on ~97% of items. Tri-state: true / false / null (unverified) — 0.8% / 77.0% / 22.2% of the live board. Addendum in DT4-WINDOW-GATE-FINDINGS.md | `read-window-range --profile`, `↳ diurnal:` (formatTimedLap), `softBuyRead`/`formatSoftBuy`, the `/scan` digest soft-buy cell, `read-schedule.mjs` | Shipped. ~99% of window renders became an honest absence — more than the ~90% forecast here. FOLLOW-UP ~~left open~~ **CLOSED by DT4c** (2026-08-10): `clean` no longer gates the app's Trends ★ badge — that badge is now on `windowReliability` and `hourConcentration` was removed from `js/trends.js` outright. This clause read as still-open for the length of one commit |
| DT4b | ✅ **SHIPPED 2026-08-10** — **Render the window the gate verified** | Closes the fit-window transfer gap DT4 flagged: the gate judged 14 days while the surfaces rendered a 7-day fit (agree on the dip hour 34.4%, on the span 18.8%). `displayFitNights` (js/windowread.mjs) refits the WHOLE lap at the gate's window when — and only when — the gate passes; non-passers byte-identical (checked over 8,000 lap pairs, 0 differ). Deleted the `prof` parameter from `pushSoftBuy`/`digestSoftBuy` so a basis mismatch is unrepresentable. **Adversarial review then found three things this chunk got wrong, all fixed in the same pass:** (1) the claim "the hours rendered are the hours verified" is FALSE — the gate correlates SHAPE VECTORS and never compares hours; the two parity halves agree on the dip hour only 25.8% of the time. The true, weaker claim: the fit now uses the same DAYS the gate examined and is measurably closer to it (mean Jaccard 0.475 vs 0.352). (2) a REGRESSION — `diurnalPhase` is a pure function of `peak.startH/endH`, so leaving it on the 7d fit made the ⏲ token contradict the peak window printed beside it (id 6034: "peak 13:00–18:00 · ⏲ post-peak" at 12:00, an hour BEFORE its own peak opened). Fixed by `phaseFromLap`, which reads the rendered window structurally. (3) five doc statements saying the gate "never moves a level" — it does, on passers (soft-buy floor moved on 23/31, max 55%) — plus a stale comment at `watch-positions.mjs` where a held-lot EXIT price silently moved | `formatTimedLap`, `softBuyRead`, the `/scan` digest soft-buy + phase cells, `watch-positions` thesis exit, `suggestlog` (now records `fitNights`) | Shipped. Both of this row's OPEN items are now CLOSED: the marker was reworded to "MAY repeat most days" (Ben's call, 2026-08-10 — we should not claim what we have not verified), and the routing convention was closed by **DT4c** below |
| DT4c | ✅ **SHIPPED 2026-08-10** — **Close the routing gap; stop the app disagreeing with the console** | The two findings DT4b's review raised but did not fix. (1) ROUTING: `read-schedule.mjs` now ROUTES through `displayFitNights` (`PROFILE_NIGHTS` reverted to its own literal after review — pinning it to the gate constant would move the levels of the ~99% of NON-passers, which displayFitNights own header forbids; one `windowReliability` call instead of two; the write-only `series` map deleted), while `read-window-range.mjs` deliberately DISCLOSES instead — its window is the caller's explicit `--nights`, so silently refitting it would make the flag a lie. New pure, mutation-tested `fitWindowMismatchNote` gives the rule one home and two renderings. (2) THE APP: the Trends `★` gated on `hourConcentration.clean`, a predicate the SAME DT4 study measured as a NON-discriminator (clean=true +5.0pp/+4.4pp n=60 vs clean=false +3.6pp/+4.7pp n=1919 — no gap, backwards on the peak side); it is now gated on `windowReliability`, and `hourConcentration` was REMOVED from `js/trends.js` rather than demoted to flavour text. A plain-language `Timing check:` line now renders ALWAYS. **A third surface surfaced mid-fix**: the lookback toggle offered only 7d/28d, so the app's fit NEVER matched the gate's 14d window — 14d added and defaulted | `read-schedule.mjs`, `read-window-range.mjs`, `js/trends.js`, `js/windowread.mjs` (new export), `APP_VERSION` 0.73.0 | Shipped. Badge is now RARE by construction (~0.8% of items pass the gate). **Firing rate corrected after review:** the badge AS SHIPPED ran at the 7d default, so the honest before/after is **5.66% → 0.68% (~8x, full archive, n=3,800)**, not the 2.1% → 0.5% first recorded here — that compared the old predicate at the NEW 14d lookback, holding constant the very thing this chunk changed. Reliability's own lift evidence is n=8 — Ben can revert the rate. Driven in a real headless browser, since `js/*.js` is outside `check-imports` and CI's smoke never opens an item. STILL NOT machine-enforced: a NEW surface can still fit its own window silently |
