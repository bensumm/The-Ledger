...
| F-E | PLANNED (wave 2) | — | expose `AMP_ASK_Q`/`AMP_BID_Q` (default 0.5, KEEP) as `amplitudeRanges` opts + a CLI flag + log them in `amplitudeShadow` so an experiment run is distinguishable in the ledger (feeds F-G). Console-only, no APP_VERSION. |
| F-F | PLANNED (wave 2) | — | trough-vs-decay DISPLAY annotation on the amplitude reach cell — a 3-signal classifier (`osc.oscillating` + `dae.floorSlope` sign + `driftShadow.margin` sign, all already in scope at `screen-flip-niches.mjs`~1603-1614, no new compute): oscillating+floor≥0 → "trough phase — buy"; oscillating+floor<0 → "oscillating into a falling floor — margin clears/doesn't"; knife → "no real cycle to harvest" (DIRECTION-AGNOSTIC — verified the Aldarium rising-hollow mirage also lands in `knife`, so the bucket must NOT say "decay"). Console-only, no APP_VERSION. |
| F-G (was #3) | PLANNED (wave 2) | — | REAL-fill amplitude retro. The AGGREGATE already works with ZERO code (`HORIZON_BY_MODE.amplitude=2d` + `aggregateOutcomes` per-niche → `analyze-record`'s rollup shows a real amplitude row the moment one round-trip closes; fills/positions schema has NO missing field — the nearest-prior-suggestion join handles it). BUILD: a ONE-LINE enrichment `base.amplitude = s.amplitude` (`retrojoin.mjs`:153) + a small `analyze-record` section printing shadow(ampBid→ampAsk±drift) vs realized(buy→sell net) + the discount, n-gated (reuse existing `MIN_N` floors). Deploy-small-to-learn posture doc'd; CONCLUSION gated on real closed cycles (tuition). No fork, no APP_VERSION. |
| F-H (was #5) | PLANNED (wave 2) | — | detector short-window fix. `AMP_NIGHTS=14` feeds BOTH the gate (`amplitudeRanges`) AND the detector (`oscillationVsKnife(stats.days)`) from the same `stats` — REQUIREMENT: a SEPARATE `OSC_DETECTOR_NIGHTS` (> AMP_NIGHTS, own trailing window) so the detector gets ≥1.5 cycles WITHOUT widening the gate's recency read (SIGNAL-RECENCY regression guard). Still n≈0. No APP_VERSION. |
