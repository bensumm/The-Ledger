...
| F-B | ✅ LANDED | 62da022 | amplitude Stage-1 fetch pool gains a WATCHLIST RESERVE (`gatecandidates.mjs` + `admission.mjs`, both fetch-pool paths) — bypasses `AMP_STAGE1_MIN_PCT` + guarantees a slot below `AMP_TOP_DEFAULT` for a `watchlist.json` id. Live-proven: Toxic blowpipe reached the gate and passed on its real margin. Pinned by new fixtures in `gatecandidates.test.mjs` + `admission.test.mjs`. **CAVEAT: only helps WATCHLIST members** — fang + dragon boots are NOT on `watchlist.json`, so they still don't surface; add them to reserve them a slot. SEPARATE follow-up (F-D, not built): a general fetch-WIDEN (e.g. `AMP_TOP_DEFAULT` 25→40) — a main-session read-only run at top-60 surfaced 5 more big-tickets (Virtus set/robe, Oathplate, Tormented synapse) the top-25 cut hides, but on the verify trio they're the same thin sub-1% story (Virtus set verified +0.48%/cycle off the MEDIAN peak), so widening surfaces the class without manufacturing edge — worth shipping for visibility, costs ~+15–35 fetches/scan. |
