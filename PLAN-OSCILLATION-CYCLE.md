...
| F-E | not built | — | expose/raise `AMP_ASK_Q` (currently 0.5 = median daily high) to quote nearer the ACTUAL peak — the reach-vs-margin dial. Ben 2026-07-22: keep at 0.5 for now. |
| F-F | not built | — | the amplitude table's `both-leg reach (recent-3)` column is PHASE-BIASED against a trough-phase oscillator (3-day window < ~7-8d cycle → reads low during the down-half, which is the BUY point; blowpipe live: ask full 6/14 but recent 0/3 = trough phase, not decay). Refine to a full-cycle or phase-annotated reach so it stops mis-flagging trough-phase oscillators. The drift-adjusted margin gate already handles the phase correctly; this is the DISPLAY-column fix. |
