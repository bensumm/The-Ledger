...
- `js/` — app logic as ES modules: `state.js` (shared mutable state as one `STATE`
  ...
  **`windowReliability`** (DT4, 2026-08-10 — the split-half hours gate: parity-split the last
  ...
  per-day HIGHS/LOWS with ZERO new fetch; `realityClause` is the ONE renderer for the three console surfaces
  (`--profile` / `formatTimedLap` / `quote-items` `windowExit`). Constants `SPIKE_REACH_FRAC`/`SPIKE_PLACEMENT_PCTILE`/
  `SPIKE_MIN_GAP_FRAC`/`REALITY_TYPICAL_QUANT`/`REALITY_TYPICAL_RECENTN` are PLACEHOLDERS; inform-only, n≈0,
  console-only, never gates — the emitted `level` is unchanged) + **`hourConcentration`/`diurnalTimedLap`** (PLAN-DIURNAL-TIMING DT1
