...
| AC6 | churn buy leg → 'band-low' anchor; delete dead reach-fold buy branch | ✅ SHIPPED core; the churn buy-cell pXX DISPLAY (screen-flip hunk) defers with the read-window-range/WIP cleanup |
| AC7 | band sell fold KEPT in discovery (crux verdict) — doc note + exit path | ✅ SHIPPED (reach-fold.mjs header note) |
| AC8 | fold-as-datapoint in read-window-range (`fold:` line + `result.fold` + `--niche`) | IMPLEMENTED, DEFERRED — entangled with read-window-range's uncommitted `--profile`-compose WIP; lands with the WIP-pile cleanup |
| AC9 | rank guardrail: (a) test pin + (b) overnight-sort ask-reach factor | (a) ✅ SHIPPED (estimators.test pin); (b) HELD for refinement (overnight reorder had rank-0 rows high — separate Opus path, not landed) |
