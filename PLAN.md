...
| **SLT** | Sale-log tax fix (folded from `plans/PLAN-SALE-LOG-TAX.md`, shipped 2026-09-01): C1 (worthNet flag at ingest — `.json`-era sell `worth` is NET of GE tax) → C2 (net-primary matchTrades + `sellNetEach`, `grossFromNet` display inverse in quotecore) → C3 (warn-only per-file convention audit) → C4 (real-book acceptance + docs). Design home: `pipeline/FILLS-PIPELINE.md` §5.1 + the reconstruct.mjs header; full text `git show d1a6516:plans/PLAN-SALE-LOG-TAX.md`. |
