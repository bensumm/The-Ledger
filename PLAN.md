...
- **Thin-reserve should scale with `--capital` (screen fetch-admission, `pipeline/lib/signal/admission.mjs`
  ...
  **Sibling: MT2's `GEAR_RESERVE` (Status table above, shipped 2026-07-27)** — same function, a DIFFERENT
  ...
  scan can be read; whoever does this entry should scale BOTH reserves together off the same `--capital`.
- **Class B mid-tier: does the `FLOOR` 50 → 3,500 recalibration overshoot the mid band? (from
  `PLAN-MID-TIER-ADMISSION.md`, folded 2026-07-27 — MT1–MT3 did NOT address this.)** Berserker helm (780/d)
  and Dragon scimitar (1.7k/d) never become candidates at all: admission needs `limitVol ≥ FLOOR (3,500/d)`
  OR `limitVol × mid ≥ GP_FLOOR (4.5b)`, and those paths cross at mid ≈ 1.29m — above it the gp-flow path
  eases as price rises, below it the flat 3,500/d binds. Mid-tier gear is the one class **too cheap for the
  gp-flow path and too slow for the volume path**. `FLOOR` was count-matched against the corrected
  rolling-24h distribution in PLAN-VOL24 step 2, so this may be correct — 780/d genuinely may be too thin to
  want. Deliberately NOT a code chunk: answering it needs the corrected volume distribution across the mid
  band plus an outcome join, i.e. an `/analyze`-shaped study, not a threshold tweak (rule 4).
