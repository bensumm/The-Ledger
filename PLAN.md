...
| V2-P0 | Context chain + unified held verdict | new `pipeline/lib/context.mjs`, `quote.mjs`, `watch.mjs`, tests | OPEN |
| V2-P1 | Surface extraction + replay harness | `screen.mjs`, new `lib/gatecandidates.mjs`, `surviveMode`, fixtures | PARTIAL — mechanical parts ✅ `f02fbf5` (gateCandidates/rankAndSlice/surviveMode → `lib/gatecandidates.mjs`, live-diff byte-identical, `survivemode.test.mjs` pins PRE-amendment falling behavior for the P5 re-pin); replay harness OPEN (needed D0's fixture format, now unblocked) |
| V2-P2 | Validate stage + reachValidator (every surface) | new `js/validate.mjs`, `js/windowread.mjs` (move), `screen.mjs`, `quote.mjs` | OPEN |
