...
- **med (UX)** — a freshly-logged manual entry for a NON-watchlisted item is silently swallowed by the "Watchlist only" ledger filter (`renderLedger` line ~395 filters `fillsPending` by `ledgerWatchOnly`), so the optimistic pending row never appears and the write looks like it did nothing (hit live 2026-07-03: Feather/Dragon-scim test adds). Pending rows are the user's just-taken action — they should always render regardless of the watch filter, or show a "N pending hidden by watchlist filter" hint. Fix in chunk 4 (small `renderLedger` change).
- **note (not a code bug)** — the app's "Link fills log…" produced `coffer-manual.log.txt` (a 2nd manual file) when Ben created-new during the picker; `sync-fills.mjs` ingests `.txt` too so it wasn't broken, but app-writes (`.txt`) and CLI-writes (`.log`) split across two files. Resolved by re-linking to the existing `coffer-manual.log`. Consider: `linkFillsLog` could suggest/pin the canonical filename, or a doc note to always pick the existing file.
