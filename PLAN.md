...
- **WEEKDAY-LEVEL — a measured weekly price cycle the tool cannot express (CLOSED-NEGATIVE 2026-09-08, pending owner sign-off; written up as `plans/PLAN-WEEKLY-CYCLE.md`, prefix WK. WK1 EXECUTED — null: no weekday structure in the lane's own entry record. WK2 EXECUTED — the program-deciding universe scan fired branch (c) on its corrected rerun: ZERO FDR discoveries of 2,516 items — measured, too thin, don't build. ⚠ WK2's FIRST run reported branch (a)/193 discoveries; that was an instrument artifact (the surrogate null skipped the MA15 detrend), caught by adversarial review and retracted in the plan + script header — do not quote first-run numbers. Honest residue = an inform-only annotation on the locked weekly set at p<0.05 (Old school bond trough Sat p=0.0014; Mole skin trough Sun p=0.0050; blowpipe trough Tue p=0.036 — with fang suggestive-only at p=0.060, exact §4 Tue agreement but ≈0.106 under the round-2 persistence-matched null). Do NOT re-dispatch any WK measurement; a second archive era is the only thing that reopens this).**
