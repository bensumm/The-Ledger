...
| V2-P6b/c | TTF estimators + per-thesis ranking (net × P(fill) ÷ TTF) + sub-floor fallback | `js/strategies.mjs`, `lib/gatecandidates.mjs`, retro-join feed | OPEN (see P6 rulings above; price-basis principle: ONE price pair per suggestion — the thesis's own recommended bid/ask — with net, P(fill), and TTF all evaluated at that same pair; realized-vs-suggested slippage per thesis from the retro-join is the calibrator) |
