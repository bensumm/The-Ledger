...
- **WEEKDAY-LEVEL — a measured weekly price cycle the tool cannot express (OPEN, found 2026-09-08; written up as `plans/PLAN-WEEKLY-CYCLE.md`, prefix WK. WK1 EXECUTED 2026-09-08 — null: no weekday structure in the lane's own entry record; do NOT re-dispatch. WK2 EXECUTED 2026-09-08 — the program-deciding universe scan fired pre-registered branch (a): per-item multi-day LEVEL structure is real (83 items clear every floor) but the WEEKLY-locked class is near-empty (5 of 2,516; bond/fang/blowpipe) and the market is NOT one index — the §4 class story mostly did not survive; measurements 2+4 revive in DERIVED-WINDOW form, weekday buckets are dead. See the plan's WK1/WK2 result sections; do NOT re-dispatch either measurement).**
