...
// --- askReachDecayNote (PLAN-DIURNAL-TRIAGE DT3) acceptance ---------------------------------------
// Replaced the hourlyDriftNote block when the per-hour slope was deleted (see hourly-lmh.mjs's
// tombstone). The ask-decay clause these fixtures exercise is unchanged — it was the surviving half of
// the old note and is now the whole note.
ok('askReachDecayNote: a decaying ask reach renders the per-day rate walk', () => {
  const decay = { perDay: [{ date: 'd1', hoursReached: 18, hoursLogged: 24, frac: 18 / 24 }, { date: 'd2', hoursReached: 11, hoursLogged: 24, frac: 11 / 24 }, { date: 'd3', hoursReached: 4, hoursLogged: 15, frac: 4 / 15 }], decaying: true };
  const text = askReachDecayNote(decay, { ask: 25_300_000, fmt: n => (n / 1e6).toFixed(1) + 'm' });
  ...
  assert.equal(text, "ask-reach decay: ask 25.3m reached 75%→46%→27% of each day's hours (sliding under)");
});
ok('askReachDecayNote: a NON-decaying read renders nothing (silence, not a padded null clause)', () => {
  const decay = { perDay: [{ date: 'd1', hoursReached: 3, hoursLogged: 24, frac: 3 / 24 }, { date: 'd2', hoursReached: 5, hoursLogged: 24, frac: 5 / 24 }], decaying: false };
  assert.equal(askReachDecayNote(decay, { ask: 1000, fmt: String }), null);
});
...
// hourlyDriftNote rendered the per-hour slope read, deleted 2026-08-09 as a measured non-signal.
ok('hourlyDriftNote stays deleted', () => {
  assert.equal(WR.hourlyDriftNote, undefined, 'the drift-note renderer must not come back');
});
