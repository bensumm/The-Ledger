 ...
 *
 *   node pipeline/commands/run-loop.mjs [--watch <min>|off] [--scan <min>|off] [--min-idle <gp>] [--no-sync]
 *
 ...
 *   sync  — `node pipeline/commands/sync-fills.mjs --local` : rebuild fills/positions/offers.json from the exchange
 ...
 *   watch — `node pipeline/commands/watch-positions.mjs`         : the position/offer deterioration pass (every --watch min).
 *   scan  — `node pipeline/commands/screen-flip-niches.mjs --mode all` : opportunity discovery (every --scan min), GATED on
