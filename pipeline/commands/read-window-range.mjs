...
if (W_MODE === 'fixed') {
  const wm = W_RAW.match(/^(\d{1,2})-(\d{1,2})$/);
  if (!wm) { console.error('error: --window expects local hours like 0-8 or 23-7, or the literal `peak` / `dip`'); process.exit(1); }
  [W_FIXED_START, W_FIXED_END] = [parseInt(wm[1], 10), parseInt(wm[2], 10)];
  if (W_FIXED_START > 23 || W_FIXED_END > 23 || W_FIXED_START === W_FIXED_END) { console.error('error: --window hours must be 0-23 and distinct'); process.exit(1); }
}
...
function fiveMinStats(id, wStart, wEnd) {
  ...
  return windowStats(mapped, { nights: NIGHTS, wStart, wEnd });
}
...
for (const want of positionals) {
  ...
  let W_START = W_FIXED_START, W_END = W_FIXED_END, wResolvedFrom = null;
  if (W_MODE !== 'fixed') {
    const w = prof ? prof[W_MODE] : null;
    if (w) { W_START = w.startH; W_END = w.endH; wResolvedFrom = W_MODE; }
    else log(`\n  ⚠ --window ${W_MODE}: too thin to derive a diurnal ${W_MODE} window — falling back to ${pad2(W_FIXED_START)}:00–${pad2(W_FIXED_END)}:00 local`);
  }
  ...
  if (A.profile !== undefined) {
    ...
    if (!prof) {
      ...
      // 34.4% of the time and on the full span 18.8%. Every other surface closes this by refitting to
      ...
      const winScore = (w, side, userLevel) => {
        if (!w) return null;
        const ws = windowStats(series, { nights: NIGHTS, wStart: w.startH, wEnd: w.endH });
        if (!ws || !ws.days.length) { log(`    ↳ in-window: too thin to score these hours`); return null; }
        const own = recencySplit(ws.days, side, w.level, RECENT_NIGHTS);
        const pool = side === 'ask' ? ws.medVolHi : ws.medVolLo;
        log(`    ↳ in-window: this level reached ${rsTxt(own)} · pool ${fmt(pool)} u (what a resting ${side} competes for IN THESE HOURS, not all-day)`);
        let user = null;
        if (userLevel != null) {
          const u = recencySplit(ws.days, side, userLevel, RECENT_NIGHTS);
          const gap = userLevel - w.level;
          const dir = gap === 0 ? 'AT' : gap > 0 ? 'ABOVE' : 'BELOW';
          log(`      your --${side} ${fmt(userLevel)} sits ${gap >= 0 ? '+' : '−'}${fmt(Math.abs(gap))} ${dir} this window's own level · in-window reached ${rsTxt(u)}`);
          user = { level: userLevel, reach: u, gap };
        }
        return { level: w.level, window: { startH: w.startH, endH: w.endH }, reach: own, pool, user };
      };
      log(`  DIP window ${fmtHourRange(prof.dip.startH, prof.dip.endH)} — recent level ${fmt(prof.dip.level)}${dipRC ? ' ' + dipRC : ''}`);
      const dipScored = winScore(prof.dip, 'bid', BID);
      log(`  PEAK window ${fmtHourRange(prof.peak.startH, prof.peak.endH)} — recent level ${fmt(prof.peak.level)}${peakRC ? ' ' + peakRC : ''}`);
      const peakScored = winScore(prof.peak, 'ask', ASK);
      result.windowScored = { dip: dipScored, peak: peakScored };
  ...
  if (A.profile !== undefined && prof && W_MODE === 'fixed'
    ...
    log(`  ⚠ this --window (${winLabel}) is NOT the diurnal PEAK ${fmtHourRange(prof.peak.startH, prof.peak.endH)} or DIP ${fmtHourRange(prof.dip.startH, prof.dip.endH)} printed above — every reach/placement/volume figure below describes THESE hours only. For the peak/dip, re-run with --window peak (or --window dip).`);
    result.windowDivergesFromProfile = true;
  }
  ...
  const fiveStats = (BID != null || ASK != null || EXIT != null) ? fiveMinStats(r.id, W_START, W_END) : null;
  ...
  const logReachMargin = (rm) => {
    ...
    // HF2: the predicate is the SHARED reachMarginTrigger (matrix-pinned equivalent to the old inline one).
    if (reachMarginTrigger(rm) === true) {
...
if (JSON_OUT) console.log(JSON.stringify({ kind: 'windowrange', generatedAt: new Date().toISOString(), nights: NIGHTS, window: W_MODE === 'fixed' ? { start: W_FIXED_START, end: W_FIXED_END, mode: 'fixed' } : { mode: W_MODE }, items: results }, null, 2));
