...
// Bonds ARE in the app catalog (js/market.js `bondMarginOpts`); estimators/pair/reach/validate opt in via
// `row.bond` (computeQuote sets it), the screen lanes (amplitudescreen/flip-niches) via `{bond, guide}`.
...
// ignored-items.json and quarantineEvents drops it BEFORE reconstruction (verified: zero positions.json
// rows). Twin of the matchTrades gap noted in reconstruct.mjs; fixing either needs a guide price at the
// P/L surface and moves historical realised P/L.
