...
export const DL4_MIN_GP_FLOW = 9_000_000;       // PLAN-VOL24 step 2: 500k → 9m. ⚠ PROVENANCE CORRECTED 2026-08-10: this said "count-matched to CORRECTED rolling-24h gp-flow", which is FALSE and self-contradictory (the same comment then described a flat scale). It was NOT count-matched — it is DL4's own 500k multiplied by GP_FLOOR's ~18× factor (PLAN-VOL24 §Step 2 table records exactly that), and a flat multiplier is the method this plan elsewhere REJECTS. ⚠ THAT CORRECTION ITSELF OVERSTATED, fixed 2026-08-11: it went on to say the ~18× "came from the ~10–27× /24h under-report, which is now retracted", and therefore that this constant has NO empirical anchor. Both halves overshoot. The ~18× is the OUTPUT of GP_FLOOR's own count-match (§Step 2 records `GP_FLOOR | 250m | 4,500m | count-matched gp-flow (89; ~18×)` — the ratio is what the count-match produced, not an input taken from the retracted ratio), and it still checks out: GP_FLOOR 4.5b admits 91 today against that target of 89. So the accurate claim is narrower — this constant was TRANSFERRED from GP_FLOOR's count-match and was never count-matched on its OWN distribution. That is a real weakness, just not "no anchor". ⚠ AND THAT CORRECTION OVER-CORRECTED IN TURN (fixed 2026-08-11): it claimed the constant "can no longer BE count-matched by the original method", because the target came from a July legacy distribution that no longer exists. That argument proves too much — the SAME July-legacy targets (884/361/438) are used as valid checks elsewhere in this very wave. A count-match target is a COUNT OF ITEMS TO SURFACE, and a count survives a change of basis; what is invalid is re-SOLVING on today's legacy distribution, which is what produced the ~424k figure (today's legacy gp-flow ≥ 500k admits 2,194; the rolling threshold admitting 2,194 is ~414k). The real obstacle is duller and is a BOOKKEEPING gap: PLAN-VOL24 §Step 2 records DL4's basis as "GP_FLOOR's ~18× applied to DL4's own 500k" with NO target count — the only row in that table lacking one. So its July target was never written down. Re-anchoring is a judgment call about what count you want surfaced, not an epistemic impossibility. What IS honest to state meanwhile: the shipped 9m admits ~1,214 items on the rolling basis. NOT tied to MIN_GPD's NET-throughput floor (250k since 2026-08-08), a different dimension. Node-only (dip nomination) → no APP_VERSION bump. Min mid×limitVol gp-flow/day to be worth watching
